...
The tolerance, the seeds and the pass criterion were fixed before generation and
never adjusted afterwards. **The trial budget is the one exception, and it is
disclosed rather than buried:** the harness and the pytest gate default to 10,000
trials per seed, and the reported 37-case run was capped at 3,000 per seed —
9,000 vectors per case — to fit the event's clock. The cap was applied identically
to both arms, before either arm was scored, and it can only make the criterion
*easier*, never harder.
...
| **16 · Document-claim check** | The claim verifier compared README.md against `results/` and nothing else. A pre-submission audit read every other document against the same artifacts and found the *same* figures had drifted again in four of them — a 14-workbook corpus, a 10-case self-test, a port inventory frozen at the pre-growth count, and a present-tense limitations paragraph still sized to the pre-growth 10-case corpus, three sections below the 37-case headline — while all six checks stayed green. The gap was never the checking, it was the *surface* being checked. | **16 superseded claims now scanned across all 6 prose documents**, plus a placeholder-and-leak scan of the 2 transcripts. Reintroducing one stale claim turns the check red and `witness.verify` exits 1 — demonstrated, not asserted. | **Kept.** The rendered transcripts are deliberately exempt from the superseded-claim scan: a transcript is supposed to contain numbers that were later corrected, and editing one to satisfy a linter would falsify the deliverable it exists to provide. |
| **17 · Final comparison** | Both arms, 37 cases, 3 seeds, 3,000 vectors per seed (9,000 per case).                                                                                                                                                      | See [Final result](#final-result)                                                                                                        | —                                                                                                                                                                                                                                                                        |
...
Second failure mode, stated plainly: **the corpus is too easy, not too small.**
37 cases clears the PDF's ten-or-more bar comfortably, so count is no longer the
problem — difficulty is. Eleven of the 37 have a single free input, and the
repair loop fired roughly once across the entire twelve-case ablation set (mean
repairs 0.08). That is precisely why the ablation could not separate the repair
signals: you cannot compare two ways of repairing a port on a corpus that
almost never needs repairing.

The highest-value next investment is therefore **harder cases, not more of
them** — deep cones with rounding, tier boundaries and date semantics, selected
because a first-draft port fails them. Adding cases of the present difficulty
would grow the sample without growing the discrimination.
